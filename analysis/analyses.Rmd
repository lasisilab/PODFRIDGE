...
```{r}
# Improved function to calculate false-positive rates for each relationship type
calculate_false_positive_rate <- function(input_df, input_df_unrelated, population, relationship_type, m_value) {
  
  # Filter the input dataframe for the specific population and relationship type combination
  input_df_filtered <- input_df %>%
    filter(population == population,
           known_relationship_type == relationship_type,
           tested_relationship_type != "unrelated")

  input_df_tested_unrelated <- input_df_unrelated %>%
    filter(population == population, tested_relationship_type == relationship_type)

  # Calculate the number of simulations for the relationship type
  n_sims_relationship_type <- nrow(input_df_filtered)

  # Calculate the cut value from unrelated log_R_sum values
  cut_value <- quantile(input_df_tested_unrelated$log_R_sum, 1 - m_value)

  # Calculate the false-positive rate for the relationship type
  false_positive_rate <- mean(input_df_filtered$log_R_sum > cut_value)
...
```{r}
# Define a vector of 'm' values
m_values <- c(1, 5, 10) / nrow(input_df %>% filter(known_relationship_type == "unrelated"))

population_groups = unique(input_df$population)
relationship_types = unique(input_df$known_relationship_type)

# Calculate false-positive rates for each combination of population group, relationship type, and 'm' value
false_positive_rates <- expand.grid(population = population_groups,
                                    relationship = relationship_types,
                                    m_value = m_values) %>%
  mutate(false_positive_rate = pmap_dbl(list(population, relationship, m_value),
                                        ~ calculate_false_positive_rate(input_df,
                                                                        input_df_unrelated,
                                                                        ..1, ..2, ..3)))
