...
p_grandpar_gen <- calc_grandparent_gen(1950)

N <- calc_final_N(US_pop_fp, p_grandpar_gen)
# N <- calc_final_N(US_pop_coop_fp, p_grandpar_gen)


N_coop <- calc_final_N(US_pop_coop_fp, p_grandpar_gen)

```

```{r pop-size}
population_size_plot <- function(N) {
  populations <- N %>% 
    pivot_longer(-Year, names_to = "Population", values_to = "N")
  
  ggplot(populations, aes(x = Population, y = N)) +
...
population_size_plot(N)
population_size_plot(N_coop)
```
...
# Function to calculate the probability of having at least one p-th degree cousin
calc_prob <- function(p, DB.size, N) {
  prob.no.rellys <- unlist(sapply(DB.size, function(db_size) {
    exp(-2^(2*p - 2) * db_size / N)
  }))
  return(1 - prob.no.rellys)
}

# Function to calculate the number of p-th degree cousins for different database sizes
calc_cousins_db_sizes <- function(p, DB.sizes, populations_df) {
  data <- data.frame(p = integer(), Population = as.character(), DB.size = double(), Cousins = double())

  for (pop_name in unique(populations_df$Population)) {
    N <- populations_df$N[populations_df$Population == pop_name]
    for (j in 1:length(DB.sizes)) {
      DB.size <- DB.sizes[j]
      cousins <- calc_cousins(p, DB.size, N)
      temp_df <- data.frame(p = p, Population = rep(pop_name, length(p)), DB.size = rep(DB.size, length(p)), Cousins = cousins)
...
```{r}
# Calculate the number of cousins for each population and database size
cousins_data <- calc_cousins_db_sizes(p, DB.sizes, N)
