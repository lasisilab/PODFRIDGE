...
```{r}
# Define the populations for the simulation

populations <- c("all", "AfAm", "Cauc", "Hispanic", "Asian")
...
```{r}
# Combined function to simulate genotypes and calculate kinship
process_row <- function(row, allele_frequency_data, kinship_matrix) {
...
for (i in 1:nrow(individuals_genotypes)) {
  individuals_genotypes[i, ] <- process_row(individuals_genotypes[i, ], df_allelefreq, kinship_matrix)
}
...
list_of_rows_approach <- function() {
  rows_list <- split(individuals_genotypes, seq(nrow(individuals_genotypes)))
  final_rows_list <- future_map(rows_list, ~process_row(.x, df_allelefreq, kinship_matrix), seed = TRUE)
...
list_of_rows_time <- system.time({
  list_of_rows_result <- list_of_rows_approach()
})
...
future_pmap_approach <- function() {
  final_individuals_genotypes <- individuals_genotypes %>%
    future_pmap(~ process_row(list(...), df_allelefreq, kinship_matrix), seed = TRUE) %>%
    bind_rows()
  return(final_individuals_genotypes)
}
...
future_pmap_time <- system.time({
  future_pmap_result <- future_pmap_approach()
})
...
benchmark_results <- microbenchmark(
  list_of_rows = list_of_rows_approach(),
  future_pmap = future_pmap_approach(),
