...
simulation_setup <- generate_simulation_setup(kinship_matrix, populations_list, 5,10)
...
benchmark <- microbenchmark(
  dplyr_version = process_simulation_setup(simulation_setup, df_allelefreq, kinship_matrix, loci_list, loci_lists, "data/processed_genotypes.csv", "data/summary_genotypes.csv"),
  times = 10
)
...
```{r}
# Grab simulation packages
# 
# # R script to extract library calls from another R script
extract_libraries <- function(file_path) {
  lines <- readLines(file_path)
  library_lines <- grep("^\\s*library\\(", lines, value = TRUE)
  packages <- sub("^\\s*library\\((.*)\\).*", "\\1", library_lines)
  packages <- gsub("\"|'", "", packages)
  return(packages)
}

# Replace 'your_script.R' with the path to your R script
packages <- extract_libraries("code/STR_sims.R")
writeLines(packages, "code/STR-sims-env.txt")
...
calculate_proportions_exceeding_cutoffs <- function(input_df, cutoffs) {
  ...
  proportions_exceeding <- df_with_cutoffs %>%
    ...
    summarize(
      ...
      n_related = if_else(relationship_type != "unrelated", n(), NA_integer_),
      .groups = 'drop'
    ) %>%
    filter(relationship_type != "unrelated")
