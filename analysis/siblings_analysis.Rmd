...
```{r}
child_df2 <- mother_data %>% group_by(YEAR, AGE_RANGE, RACE) %>% summarize(total_births = sum(chborn_num)) %>% ungroup()
#pivot_df <- child_df2 %>% pivot_wider(names_from = YEAR, values_from = total_births)
```

```{r}
ggplot(data = child_df2, aes(x = AGE_RANGE, y = total_births, fill = RACE)) + 
