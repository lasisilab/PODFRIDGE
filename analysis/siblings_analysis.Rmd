...
p <- ggplot(df_50_60_1960_b, aes(x = x_axis, y = norm_freq, fill = color_order)) +
  ...
  scale_fill_manual(name = "Number of Children/Siblings (Black/African American)",
                    values = black_color_mapping,
...
p <- p + geom_col(data = df_50_60_1960_w, aes(x = x_axis, y = norm_freq, fill = color_order), alpha = 0.5) +
  scale_fill_manual(name = "Number of Children/Siblings (White)",
                    ...
                    guide = "legend") +
  facet_grid(frequency_type ~ .,  # Use only rows for faceting
             scales = "free_y",   # Allow each plot to have its own y-axis scale
