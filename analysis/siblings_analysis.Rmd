...
```{r}
# Base plot with Black/African American data
p <- ggplot(df_50_60_1960_b, aes(x = x_axis, y = norm_freq, fill = color_order)) +
  geom_col(alpha = 0.5) +  # Make bars semi-transparent
...
# Add overlaying bars for White data
p <- p + geom_col(data = df_50_60_1960_w, aes(x = x_axis, y = norm_freq, fill = color_order), alpha = 0.5) +
  ...
  scale_y_continuous(labels = comma_format(scale = 1, big.mark = ",")) +
  labs(title = "Distribution of Number of Children/Siblings by Census Year",
       x = "Number of Children/Siblings",
       y = "Frequency") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
