...
## 5.Distribution of Number of Children by Age and Year(Balck/African American)
...
```{r}
mother_df <- fert_df2 %>% 
  mutate(freq_mother = if_else(RACE == "Black/African American", 
                               -freq_mother, freq_mother))
```


```{r}
ggplot(mother_df, aes(x = Age_bin, fill = RACE,
                     y = freq_mother)) + 
  geom_bar(stat = "identity")+
  labs(y = "Number of Mother", title = "Distribution of Number of Mother By Age and Census Year") + 
  coord_flip() +
  facet_wrap(~YEAR) +
