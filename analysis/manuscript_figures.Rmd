...
PROP_AA_DTC = 0.236 # TODO: need this value
PROP_EA_DTC = 0.429 # TODO: need this value
...
ea_fig <- generate_plot(rep_direct_ea, rep_or_ea, EA_POP_1990, "European Americans")
...
aa_fig <- generate_plot(rep_direct_aa, rep_or_aa, AA_POP_1990, "African Americans")
...
rep_direct_ea = rep(1, 10)
rep_or_ea = rep(1, 10)
combo_fig <- generate_plot2(rep_direct_ea, rep_or_ea, US_POP_1990, "US")
...
plot_number_matches <- function(rep_direct, rep_or, N_pop) {
    num_K = 10000
    m = 6
    min_num_seg = 2
    r = 2.5
    ...
    plot_data$Ks_proportion <- plot_data$Ks / N_pop
    
    plot_data <- plot_data[complete.cases(plot_data) & plot_data$Ks_proportion <= 0.2,]
    
    # calculated number of matches
    plot_data_matches <- plot_data %>% 
      mutate(matches = Coverage * N_pop)
    
    p <- plot_data_matches %>%
      ggplot(aes(x=Ks_proportion, y=matches/1e6, color = Relationship, group = Relationship)) +
      geom_point() +
      geom_line() +
      labs(title = "Number of Matches by Relationship vs Database Size",
           x = "Database Size / Population Size",
           y = "Number of Matches (Millions)",
           color = "Relationship") +
      theme_minimal()
...
plot_number_matches(rep(1, 10), rep(1, 10), US_POP_1990)

```

```{r database-composition1}
...
ggplot(data, aes(y=PROPS, fill=DATABASE)) + 
  geom_col(aes(x=POPULATION, group=POPULATION), position = position_dodge2()) + 
...
```{r database-composition2}

# OPTION 2
add_other <- data %>% 
  group_by(DATABASE) %>%
  summarise(PROPS = 1 - sum(PROPS)) %>%
  mutate(POPULATION = "other") %>%
  bind_rows(data)

ggplot(add_other, aes(y=PROPS, fill=POPULATION)) +
  geom_col(aes(x=DATABASE)) +
   labs(title = "Genetic database composition for European and African Americans",
         x = "Database",
         y = "Proportion of Database",
         color = "Database") +
