...
The Combined DNA Index System (CODIS) is a national database of DNA profiles maintained by the FBI. It contains DNA profiles from convicted offenders, arrestees, and forensic samples collected from crime scenes. The racial composition of CODIS has been a topic of concern, particularly regarding the overrepresentation of Black individuals compared to their population size in the U.S.
...
*   Direct data on the number of Black and White people in CODIS, from a Freedom of Information Act (FOIA) request conducted by Muprhy and Tong (2020) for the following seven states: California, Florida, Indiana, Maine, Nevada, South Dakota, and Texas. [1]
...
*   The U.S. Census data for each state [4], which provides the total population and the racial composition of the population. This data is available through the U.S. Census Bureau's API.

Based on the available data types, we separate states into three categories:\
1. States who have data available in the Murphy & Tong dataset. For these states, no calculations are needed to estimate the number of people and racial compositions in CODIS.\
2. States who have NDIS and SDIS data available but are not in the Murphy & Tong dataset. For these states, the number of people in CODIS is given by the SDIS database size, and we only need to estimate racial compositions in CODIS.\
3. States with only NDIS data available. For these states, both the number of people and racial compositions in CODIS are required to be estimated. \
...
# 2 Models to predict the number of people and racial composition of CODIS

## 2.1 Predict the racial composition of CODIS profiles

For states in categories (2) and (3), we construct a multinomial regression model using stick-breaking techniques to estimate the racial composition of the CODIS profiles. In the stick-breaking technique, we divide the US races into three types, black + white + other, and separately estimate the percent of white people in all people $p_{1}$ and the percent of black people in non-white people $p_{2}$. Then the racial breakdown is given by 
$$CODIS\_Prop_{white}=p_1, \quad CODIS\_Prop_{black}=(1-p_1)p_2,\quad CODIS\_Prop_{other}=(1-p_1)(1-p_2).$$
We use the following binomial regression models to estimate the racial composition of CODIS profiles:
...
where $g(x)=\frac{\exp(x)}{1+\exp(x)}$ is the logit link function. Consequently, the number of the corresponding racial groups in CODIS follows a binomial distribution with the trial number being the total number of people or non-white people in CODIS, and the success probability being the above estimated racial proportions. The independent variables are the racial proportions of population from the census database and the racial proportions of incarcerated people from the prison database.

## 2.2 Predict the number of people in SDIS

For states in category (3), we need to estimate the number of individuals in SDIS. To do this, we construct a regression model where the dependent variable is the number of people in SDIS, and the independent variables include: the racial composition of the U.S. Census population, the racial composition of the state’s prison population, and the number of individuals in NDIS for that state. We develop two distinct regression models—one for arrestees and one for offenders—to improve prediction accuracy:
...
The estimated number of individuals in SDIS is then calculated as the sum of arrestees and offenders: $N_{SDIS} = N_{arrestee}+N_{offender}$.


# 3 Regression to estimate the racial composition of the CODIS database.
...
response_data = read.csv(response_data_path) # CODIS data
...
# 4 SDIS regression to predict the number of people in SDIS
...
# Calculate CODIS black and white proportions using previous regression
SDIS_regression$CODIS.black = pis_mt[,1]

SDIS_regression$CODIS.white = pis_mt[,2]

SDIS_regression$CODIS.other = pis_mt[,3]
...
Using the above regression models, we make predictions of the number of people and racial composition in CODIS by state. Then the number of people for each racial group is calculated by $N_{race} = N_{SDIS} * Proportion_{race}$. The plot below shows our estimates for each state, colored by the data source used for each state. The number of Black people in the database are indicated with circles and the number of White people is indicated by triangles.  
...
We additionally generate plots showing the difference in the percent of people in CODIS versus the U.S. census percent of the population for each racial group in each state. 
...
ggplot() +
  geom_sf(data = final.data.sf$geometry, aes(fill = final.data.sf$CODIS.black-final.data.sf$census.percent.black),color = NA) + 
...
ggplot() +
  geom_sf(data = final.data.sf$geometry, aes(fill = final.data.sf$CODIS.white-final.data.sf$census.percent.white),color = NA) + 
  geom_sf(data = final.data.sf$geometry, color = "black",lwd = 0.3,alpha = 0) +
  theme_void(base_size = 11) + 
  scale_fill_viridis(option = "magma", direction = -1)+
  labs(fill = "Difference between proportion of\npeople in CODIS that are White\nand census proportion of\nthe population that is White") + 
  #theme(plot.margin = unit(rep(0.5, 4), "cm")) +
  theme(legend.key.height= unit(1, 'cm'), legend.key.width= unit(1, 'cm'),
        legend.title=element_text(size=20),  legend.text=element_text(size=20)) 
```
...
final.data.long.CODIS = final.data.sf %>%
  pivot_longer(
    cols = c("CODIS.black","CODIS.white", "CODIS.other"),
    names_to = "race",
