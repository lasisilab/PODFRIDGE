...
```{r}
filter_df <- df_proportions %>% filter(YEAR == "1990", AGE_RANGE %in% c("40-49", "50-59"))
```
...
```{r}
# Adding annotations and highlighting significant shifts
child_plot +
  # Adjust the rectangle to fit within the proportion and number of children ranges
  annotate("rect", xmin = -0.45, xmax = 0.45, ymin = -0.3, ymax = 0.3, alpha = 0.3, color = "blue", fill = "blue") 
  ...
  # Optional: Adjust the theme for better visibility
  theme_minimal() 
```
...
summary_stats <- filter_df %>%
  ...
  summarise(
    mean_children = mean(chborn_num, na.rm = TRUE),
    variance_children = var(chborn_num, na.rm = TRUE),
    zero_inflation = mean(chborn_num == 0, na.rm = TRUE)
  )
