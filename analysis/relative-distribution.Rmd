...
```
Result:In some cohorts (-Inf-1900, -Inf-1920, 1891-1900, 1911-1920, 1921-1930, 1931-1940, 1941-1950), the p-values for Black women tend to be similar to those for White women, even across census year, suggesting no significant difference in the probability of having zero children between the two groups. By contrast, cohorts -Inf-1890, -Inf-1910 present a large difference in p-value, indicating a discrepancy between two groups. 
...
ggplot(combined_result, aes(x = Census_Year, y = Test_Statistic, group = interaction(Cohort, Population), color = Population)) +
  ...
  theme(legend.position = "bottom")
```
Result: The test statistic graph shows that Black women have lower test statistics compared to White women in all cohorts, meaning the magnitude of difference in having zero children is not as extreme for Black women compared to White women in those cohorts. We can also observe a change in test statistics in certain cohort(1901-1910, 1911-1920, 1921-1930, 1931-1940) over census year. 
...
```{r}
filter_df <- merged_data %>% filter(Census_Year == "1990", Age_Range %in% c("40-49", "50-59"))
```
