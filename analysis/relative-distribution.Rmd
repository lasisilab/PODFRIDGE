...
cohort_data <- summary_table %>%
  ...
  dplyr::select(Race, Cohort, Census_Year, Best_Model)


unique_cohorts <- unique(cohort_data$Cohort)

model_summaries <- list()

for (cohort in unique_cohorts) {
  cohort_data <- cohort_data %>% filter(Cohort == cohort)
  ...
  if (n_distinct(cohort_data$Census_Year) > 1) {
    ...
    model_summaries[[cohort]] <- summary(logistic_model)
...
cohort_data <- cohort_data %>% filter(Cohort == "1901-1910")
  
  # Create a contingency table for Census Year and Best Model
contingency_table <- table(cohort_data$Census_Year, cohort_data$Best_Model)
contingency_table
```
