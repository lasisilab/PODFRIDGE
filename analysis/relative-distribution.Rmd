...
```
**_RESULT:_**The p-value for each variable is smaller than 0.05, indicating  non-significant effects on best_model fitting. 
...
**_CONCLUSION:_**
Both the logistics regression and the result visualization shows that there isn't a significant association between the predictor and the best-fitting model, ie. no certain races, age ranges, or census year are more likely to have a particular model as the best fit.
...
results_df_black <- data.frame(Cohort = character(),
                               Census_Year = integer(),
                               P_Value = numeric(),
...
cohorts <- unique(merged_df_black$Cohort)
...
for (cohort in cohorts) {
  ...
  # Loop through each unique Census Year for this cohort
  for (year in unique(cohort_data$Census_Year)) {
    
    # Filter for the specific census year
    cohort_year_data <- cohort_data[cohort_data$Census_Year == year, ]
    
    # Calculate the observed counts for women with zero children and with children
    observed <- table(cohort_year_data$chborn_num == 0)  
    
    # Skip if there are not both TRUE and FALSE values
    if (length(observed) >= 2) {
      ...
      test_stat <- chi_square_test$statistic
      
      # Store the results in the data frame
      results_df_black <- rbind(results_df_black, data.frame(Cohort = cohort,
                                                             Census_Year = year,
                                                             P_Value = p_value,
...
print(results_df_black)

```

```{r}
results_df_white <- data.frame(Cohort = character(),
                               Census_Year = integer(),
                               P_Value = numeric(),
                               Test_Statistic = numeric(),
                               stringsAsFactors = FALSE)

# Loop through white population
cohorts_white <- unique(merged_df_white$Cohort)

for (cohort in cohorts_white) {
  
  # Filter data for the specific cohort
  cohort_data <- merged_df_white[merged_df_white$Cohort == cohort, ]
  ...
  for (year in unique(cohort_data$Census_Year)) {
    ...
    # Calculate the observed counts for women with zero children and with children
    observed <- table(cohort_year_data$chborn_num == 0)  # TRUE: no children, FALSE: have children
    ...
    if (length(observed) >= 2) {
      ...
      # Store the results in the data frame
      results_df_white <- rbind(results_df_white, data.frame(Cohort = cohort,
                                                             Census_Year = year,
                                                             P_Value = p_value,
                                                             Test_Statistic = test_stat
                                                             ))
...
```{r}
results_df_black$Population <- "Black"
results_df_white$Population <- "White"

# Combine the two data frames
combined_result <- rbind(results_df_black, results_df_white)
combined_result$P_Value[combined_result$P_Value == 0] <- 1e-300

# Get the minimum and maximum p-values for the log scale
min_p_value <- min(combined_result$P_Value, na.rm = TRUE)
max_p_value <- max(combined_result$P_Value, na.rm = TRUE)

# Adjust plot with dynamic limits based on the p-value range
ggplot(combined_result, aes(x = Census_Year, y = P_Value, group = interaction(Cohort, Population), color = Population)) +
  geom_line(aes(linetype = Population), size = 1) +
  geom_point(aes(shape = Population), size = 2) +  # Different shapes for Population
  facet_wrap(~ Cohort, scales = "free_y") +
  labs(title = "P-Value Comparison between Black and White Populations",
       x = "Census Year", y = "P-Value") +
  theme_minimal() +
  theme(legend.position = "bottom") +
  scale_y_log10(breaks = scales::trans_breaks("log10", function(x) 10^x),
                labels = scales::trans_format("log10", scales::math_format(10^.x)),
                limits = c(min_p_value, max_p_value))  # Dynamically set limits


```
**_RESULT:_**
-In some cohorts (-Inf-1900, -Inf-1920, 1891-1900, 1911-1920, 1921-1930, 1931-1940, 1941-1950), the p-values for Black women tend to be similar to those for White women, even across census year, suggesting no significant difference in the probability of having zero children between the two groups. By contrast, cohorts -Inf-1890, -Inf-1910 present a difference in p-value, indicating a discrepancy between two groups. Specially, there is a increase in the p-value for balck population in cohort 1901-1910. 
-Since the p-values are plotted on a log scale, the extremely small values (e.g., 1e-300) suggest that there are significant differences between the observed and expected proportions of women with zero children across all census years for both populations.

```{r}
ggplot(combined_result, aes(x = Census_Year, y = Test_Statistic, group = interaction(Cohort, Population), color = Population)) +
  geom_line(aes(linetype = Population), size = 1) +
  geom_point(aes(shape = Population), size = 2) +  # Different shapes for Population
  facet_wrap(~ Cohort, scales = "free_y") +
  labs(title = "Test_Statistics Comparison between Black and White Populations",
       x = "Census Year", y = "Test_Statistics") +
  theme_minimal() +
  theme(legend.position = "bottom")
```
**_RESULT:_** 
-Black Population: The test statistics for the Black population are relatively stable across most cohorts, with moderate test statistics in the range of 5,000–10,000 for most census years.
-White Population: The test statistics are much higher for the White population, especially in cohorts like 1911-1920, 1921-1930, and 1931-1940, where they reach values as high as 100,000.
-For cohorts like 1891-1900, 1901-1910, and 1921-1930, the test statistics for the White population are significantly higher than for the Black population, indicating greater differences between the observed and expected proportions of women with zero children in the White population.
-In the cohort 1941-1950, there’s a large test statistic spike for the White population, but the Black population remains much lower, suggesting less variation in the observed and expected values for the Black population.

**_CONCLUSION:_**
-Based on the p-value comparison, both Black and White women show statistically significant differences in the number of children over time. However, the larger test statistics for the White population suggest that there might be more variability or greater differences in childbearing patterns among White women compared to Black women. So while both populations show significant patterns, White women appear to have more pronounced differences over time.
-The probability of having zero children does change over time for Black women. This is evidenced by the consistently low p-values across all census years, suggesting that the number of children born to Black women differs significantly from the expected proportions. The test statistics for Black women are stable but moderate, indicating that while there are significant differences, they are less pronounced compared to the White population.
...
```
**_RESULT:_** 
1.The ANOVA is not applicable for the following cohort since there is only one census year available in the dataset for those cohorts:-Inf-1910, 1941-1950, -Inf-1920, -Inf-1890, -Inf-1900, and 1891-1900
2.By looking at the ANOVA for mean and variance family size, the p-value of the rest cohorts are smaller than 0.05, indicating mean and variance family size for these cohort has significantly changed over different census years. 
...
```
**_RESULT:_** The p-value of all the "Census_Year" is greater than 0.05, suggesting no statistically significant effect on choice between best models. 
...
-change in prob_0 children: 1901-1910, 1911-1920, 1921-1930, 1931-1940 
-change in mean & variance: 1901-1910, 1911-1920, 1921-1930, 1931-1940 
...
child_plot +
  ...
  geom_rect(data = df_mirror %>% filter((YEAR == 1960 & AGE_RANGE == "40-49")|
            ...
            fill = "blue", alpha = 0.1) +  # Apply transparency
...
```
**_RESULT:_** These p-values are both extremely small (close to zero), meaning there is a very strong statistical difference between the two age groups (40-49 and 50-59) in terms of the number of children they have for both the Black/African American and White racial groups. 
...
**_RESULT:_** While both distributions show a trend toward smaller families, the sibling distribution is more spread out across different sibling counts.
