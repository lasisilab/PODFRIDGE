...
zero_inflation_model <- glm(Prob_0 ~ Census_Year + Race + Cohort, 
                            family = binomial(), data = summary_table)

# Summary of the model
summary(zero_inflation_model)
```
Result: The p-value of "Census_Year" is greater than 0.05, suggesting no statistically significance. Therefore, the probability of having zero children does not change significantly over time for the same cohort. 
...
```{r}
  ...
  family_size_model <- lm(Mean ~ Census_Year + Race + Cohort, data = summary_table)
  
  # ANOVA to check for significant differences in mean
  anova(family_size_model)
```
Result: The p-value of "Census_Year" is greater than 0.05, suggesting no statistically significance. Therefore, the mean in family size does not change over census years for the same cohort.
