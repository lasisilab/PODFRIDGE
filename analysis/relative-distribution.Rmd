...
## White population
filter_mean_w <- merged_data2 %>% 
  filter(Cohort %in% c("1911-1920") & Race == "White")

combined_data <- unique(bind_rows(filter_mean_b, filter_mean_w))

ggplot(combined_data, aes(x = Census_Year, y = Variance, color = Cohort)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  facet_wrap(~ Race) +
  labs(
    title = "Significant Changes in Variance Family Size Over Census Years",
    x = "Census Year",
