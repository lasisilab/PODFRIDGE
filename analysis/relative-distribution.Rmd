...
```{r}
# List of unique cohorts in the black dataset
...
for (cohort in cohorts1) {
  ...
  # Extract census year (assuming you want to store it along with cohort)
  census_year <- unique(cohort_data$Census_Year)
...
```{r}
# List of unique cohorts in the white dataset
...
for (cohort in cohorts2) {
  ...
  # Extract p-value and test statistic
  p_value2 <- chi_square_test2$p.value
  test_stat2 <- chi_square_test2$statistic
  
  # Extract census year (assuming you want to store it along with cohort)
  census_year <- unique(cohort_data2$Census_Year)
  
  # Store the results in the data frame
  results_df_white <- rbind(results_df_white, data.frame(Cohort = cohort,
                                                         Census_Year = census_year,
                                                         P_Value = p_value2,
                                                         Test_Statistic = test_stat2))
}
...
ggplot(combined_result, aes(x = Census_Year, y = P_Value, group = interaction(Cohort, Population), color = Population)) +
  ...
  scale_y_log10(breaks = scales::trans_breaks("log10", function(x) 10^x),
                ...
                limits = c(1e-300, 1e-4))  # Adjust log scale limits for better spacing```

```
Result:In some cohorts (like 1891-1900, 1911-1920, 1921-1930), the p-values for Black women tend to be similar to or lower than those for White women, suggesting no significant difference in the probability of having zero children between the two groups
...
ggplot(combined_result, aes(x = Census_Year, y = Test_Statistic, group = interaction(Cohort, Population), color = Population)) +
  ...
  theme(legend.position = "bottom")
```
Result:the test statistic graph shows that Black women have lower test statistics compared to White women in certain cohorts (like -Inf-1890 and 1891-1900), meaning the magnitude of difference in having zero children is not as extreme for Black women compared to White women in those cohorts.
...
```
Result(black population): 
1.The ANOVA is not applicable for the following cohort since there is only one census year available in the dataset for those cohorts:-Inf-1910, 1941-1950, -Inf-1920, -Inf-1890, and 1891-1900
2.By looking at the ANOVA for mean and variance family size, the p-value of the rest cohorts are smaller than 0.05, indicating mean and variance family size of black people for these cohort has significantly changed over different census years. 

```{r}
cohorts2 <- unique(cohort_w$Cohort)

# Loop through each cohort and apply ANOVA for Mean and Variance
for (cohort in cohorts2) {
  
  # Filter data for the specific cohort
  cohort_data_w <- cohort_w[cohort_w$Cohort == cohort, ]
  ...
  # Perform ANOVA for Mean
  anova_mean_result2 <- aov(Mean ~ factor(Census_Year), data = cohort_data_w)
  
  # Perform ANOVA for Variance
  anova_variance_result2 <- aov(Variance ~ factor(Census_Year), data = cohort_data_w)
  
  # Output the results
  cat("\nCohort:", cohort)
  cat("\nANOVA for Mean Family Size:\n")
  print(summary(anova_mean_result2))
  cat("\nANOVA for Variance in Family Size:\n")
  print(summary(anova_variance_result2))
}

```
Result(white population): 
1.The ANOVA is not applicable for the following cohort since there is only one census year available in the dataset for those cohorts:1891-1900, -Inf-1890, -Inf-1900, -Inf-1910, -Inf-1920, 1941-1950
2.By looking at the ANOVA for mean and variance family size, the p-value of the rest cohorts are smaller than 0.05, indicating mean and variance family size of white population for these cohort has significantly changed over different census years. 
