...
result_race <- df %>%
  group_by(RACE) %>% 
  summarize(mean = mean(chborn_num, na.rm = TRUE),
            variance = var(chborn_num, na.rm = TRUE), 
            total = n(),
            zero_count = sum(chborn_num == 0, na.rm = TRUE),
            zero_proportion = zero_count / total) %>%
  mutate(overdispersion = ifelse(variance > mean, TRUE, FALSE), zero_inflation = ifelse(zero_proportion > 0.5, TRUE, FALSE)) 
```
...
```{r}
result_race$model <- NA  # Create an empty column for the model

for(i in 1:nrow(result)) {
  if(result$overdispersion[i] & result$zero_inflation[i]) {
    result$model[i] <- "Zero-Inflated Negative Binomial (ZINB)"
  } else if(result$overdispersion[i] & !result$zero_inflation[i]) {
    result$model[i] <- "Negative Binomial"
  } else if(!result$overdispersion[i] & !result$zero_inflation[i]) {
    result$model[i] <- "Poisson"
  } else {
    result$model[i] <- "Check conditions"
  }
}

model_select_race <- result %>% select(RACE, YEAR, AGE_RANGE, overdispersion, zero_inflation, model)

kable(model_select_race, caption = "Overdispersion and Zero-Inflation for Different Groups")
```

```{r}
result_year$model <- NA  # Create an empty column for the model
...
model_select_year <- result %>% select(RACE, YEAR, AGE_RANGE, overdispersion, zero_inflation, model)

kable(model_select_year, caption = "Overdispersion and Zero-Inflation for Different Groups")
```

```{r}
result_age$model <- NA  # Create an empty column for the model
...
model_select_age <- result %>% select(RACE, YEAR, AGE_RANGE, overdispersion, zero_inflation, model)

kable(model_select_age, caption = "Overdispersion and Zero-Inflation for Different Groups")
```
...
## Fit a Poisson model
poisson_models <- df %>%
  group_by(RACE, YEAR, AGE_RANGE) %>%
  group_split() %>%
  map(~ glm(chborn_num ~ RACE+YEAR+AGE_RANGE, data = ., family = poisson()))

## Fit a negative binomial model
nb_model <- df %>%
  group_by(RACE, YEAR, AGE_RANGE) %>%
  group_split() %>%
  map(~ glm.nb(chborn_num ~ RACE + YEAR + AGE_RANGE, data = .))
