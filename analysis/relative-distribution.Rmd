...
```{r}
result$model <- NA  # Create an empty column for the model
...
for(i in 1:nrow(result)) {
  if(result$overdispersion[i] & result$zero_inflation[i]) {
    ...
    result$model[i] <- "Check conditions"  # Add fallback condition if needed
  }
}

model_select <- result %>% select(RACE, YEAR, AGE_RANGE, overdispersion, zero_inflation, model)

kable(model_select, caption = "Overdispersion and Zero-Inflation for Different Groups")
```
