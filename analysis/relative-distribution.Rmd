...
```{r}
colnames(merged_data2)[colnames(merged_data2) == "Race"] <- "RACE"

merged_df_b <- left_join(test_result_b, merged_data2 %>% select(RACE, Cohort, Census_Year, Mean), by = c("RACE", "Cohort")) %>% distinct()

merged_df_w <- left_join(test_result_w, merged_data2 %>% select(RACE, Cohort, Census_Year, Mean), by = c("RACE", "Cohort")) %>% distinct()

## Balck population
filter_mean_b <- merged_df_b %>% 
  filter(Cohort %in% c("1901-1910", "1921-1930") & RACE == "Black/African American")

## White population
filter_mean_w <- merged_df_w %>% 
  filter(Cohort %in% c("1901-1910", "1911-1920", "1921-1930") & RACE == "White")

combined_data <- bind_rows(filter_mean_b, filter_mean_w)

p<-ggplot(combined_data, aes(x = Census_Year, y = Mean, color = Cohort)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  facet_wrap(~ RACE, scales = "free_x") +
  labs(
