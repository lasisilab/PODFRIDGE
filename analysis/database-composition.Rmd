...
Here is the data from Bryc et al. and Zhang et al., estimating the self-identified ethnic and racial groups of participants in the 23andMe database. [DESCRIBE HERE THE DIFFERENCE BETWEEN THE TWO]
...
dtc_combined <- dtc_race_totals |> 
  ...
  mutate(Law_Enforcement_Access = ifelse(law_access == "yes", "Yes", "No"),
...
ggplot(dtc_combined, aes(x = reorder(Database, -value), y = value, fill = Law_Enforcement_Access)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        plot.title = element_text(size = 16, face = "bold"),
        plot.subtitle = element_text(size = 14, color = "dimgray")) +
...
[DESCRIPTION OF THESE GRAPHS, would be nice to make something that shows proportion of AA data LE accessible compared to EA data LE accessible]
...
p2 <- ggplot(data_23andMe |> 
               filter(Source == "Zhang2024"),
             aes(x = "", y = Proportion, fill = Ethnicity)) +
...
# Save the figure
ggsave("output/dtc_demographics_summary.png", final_figure, width = 15, height = 8, dpi = 300)
...
2. DTC Database Sizes and Law Enforcement Accessibility:
   - The total estimated number of individuals across all analyzed DTC databases is [provide total number].
   - Approximately [Z%] of the total DTC database population, or [number] individuals, is potentially accessible to law enforcement.
...
3. Estimated Ethnic Representation in DTC Databases:
   - In the total DTC database population, we estimate:
     ...
     * Approximately 6,243,671.605 African Americans
     * Approximately 1,731,255.712 Latinos
   - In the law enforcement accessible portion:
     * Approximately [Number] European Americans
     * Approximately [Number] African Americans
     * Approximately [Number] Latinos
