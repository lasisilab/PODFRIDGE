...
output_file2 <- file.path(output_dir, paste0("sim_combined_genotypes2",slurm_job_id,".csv"))
input_file <- file.path(output_dir, paste0("sim_processed_genotypes_",slurm_job_id,".csv"))
input_dir <- file.path(paste0(t,"/temp"))
...
individuals_genotypes <- fread(paste0(getwd(),"/",input_file))
individuals_genotypes[,5:8] <- lapply(individuals_genotypes[,5:8],as.character)
names(individuals_genotypes)[1]<-"population_tested"
...
calculate_likelihood_ratio <- function(allele_frequency_data) {
  ...
  allele_frequency_data$LR_population_known<-ifelse(allele_frequency_data$shared_alleles == 0, allele_frequency_data$k0,NA)
  ...
  allele_frequency_data$LR_population_known<-ifelse(allele_frequency_data$shared_alleles == 0, allele_frequency_data$k0,NA)
  print(summary(allele_frequency_data$LR_population_known))
  allele_frequency_data$LR_population_known<-ifelse(allele_frequency_data$shared_alleles == 1, allele_frequency_data$k0 + (allele_frequency_data$k1 / allele_frequency_data$Rxp),allele_frequency_data$LR_population_known)
  print(summary(allele_frequency_data$LR_population_known))
  ...
  allele_frequency_data$LR_population_known<-ifelse(allele_frequency_data$shared_alleles == 2, allele_frequency_data$k0 + (allele_frequency_data$k1 / allele_frequency_data$Rxp) + (allele_frequency_data$k2 / allele_frequency_data$Rxu),allele_frequency_data$LR_population_known)
...
kinship_calculation <- function(allele_frequency_data, kinship_matrix,df_allelefreq) {
  ...
  population_known<-rep(population,each=length_t)
  allele_frequency_data<-cbind(allele_frequency_data,population_known)

  t1<-df_allelefreq
  names(t1)<-c("A","locus","pA","population_known")
  allele_frequency_data<-left_join(allele_frequency_data,t1)

  t1<-df_allelefreq
  names(t1)<-c("B","locus","pB","population_known")
  allele_frequency_data<-left_join(allele_frequency_data,t1)
...
calculate_combined_lrs <- function(final_results, loci_lists) {
  ...
  combined_lrs <- final_results[, .(
    core_13 = prod(LR_population_known[locus %in% loci_lists$core_13], na.rm = TRUE),
    identifiler_15 = prod(LR_population_known[locus %in% loci_lists$identifiler_15], na.rm = TRUE),
    expanded_20 = prod(LR_population_known[locus %in% loci_lists$expanded_20], na.rm = TRUE),
    supplementary = prod(LR_population_known[locus %in% loci_lists$supplementary], na.rm = TRUE),
    autosomal_29 = prod(LR_population_known[locus %in% loci_lists$autosomal_29], na.rm = TRUE)
  ...
  combined_lrs <- melt(combined_lrs,
                       ...
                       variable.name = "loci_set", value.name = "LR_population_known")
  return(combined_lrs)
}
